...
public class JwtFilter  extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (redisUtils.hasKey(AuthConstant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {

            // Redis中RefreshToken还存在，获取RefreshToken的时间戳
            String currentTimeMillisRedis = redisUtils.get(AuthConstant.PREFIX_SHIRO_REFRESH_TOKEN + account).toString();
            ...
            if (jwtUtils.getClaim(token, AuthConstant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                redisUtils.set(AuthConstant.PREFIX_SHIRO_REFRESH_TOKEN + account, currentTimeMillis, Integer.parseInt(refreshTokenExpireTime));
